...
while (1) { //   lets play with the heater..
  ...
  TemperatureD14 = (252. * VoltageD14) / D14MEASUREDVOLTAGE;  //22.9 C is fluke readking
...
#if 0 // all work 6/14/19 ! yea    // lets test ALL the outputs,18,19,21,22,26 . should blink them all
while (1) {
  pinMode(18, OUTPUT);    pinMode(19, OUTPUT);    pinMode(21, OUTPUT);
  pinMode(22, OUTPUT);    pinMode(26, OUTPUT);
  digitalWrite(18, -state);    digitalWrite(19, -state);    digitalWrite(21, -state);
  digitalWrite(22, -state);    digitalWrite(26, -state);
  state = ~state;
